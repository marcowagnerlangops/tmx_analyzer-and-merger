...
class TMXRepair:
    ...
    @staticmethod
    def repair_record(r: SegmentRecord) -> None:
        r.source_text = TMXRepair.repair_text(r.source_text)
        r.target_text = TMXRepair.repair_text(r.target_text)
        r.source_lang = (r.source_lang or "").strip().lower().replace("_", "-")
        r.target_lang = (r.target_lang or "").strip().lower().replace("_", "-")
        if r.tuid:
            r.tuid = r.tuid.strip()
    ...
    @staticmethod
    def repair_all(records: List[SegmentRecord]) -> int:
        changed = 0
        for r in records:
            before = (r.source_text, r.target_text, r.source_lang, r.target_lang, r.tuid)
            TMXRepair.repair_record(r)
...
def run_analysis(uploaded_tmx_files, preferred_source: str, preferred_target: str, whole_word: bool) -> None:
    ...
    status.info("Repairing, classifying, checking duplicates, glossary, and QA...")
    ...
    with st.sidebar:
        ...
        if st.button("Run TMX Repair Mode", use_container_width=True):
            if not st.session_state.records:
                ...
                changed = TMXRepair.repair_all(st.session_state.records)
                st.session_state.stats = recalculate_all(st.session_state.records, whole_word)
                log(f"Repair mode completed. Updated {changed} record(s).")
